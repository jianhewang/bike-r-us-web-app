using AppSecurity;
using Backend_SalesReturns;
using Backend_Purchasing;
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddDbContext<ApplicationDbContext>(options =>
                options.UseSqlServer(
                    Configuration.GetConnectionString("eBike")));

            services.AddBackendDependencies(options =>
                options.UseSqlServer(
                    Configuration.GetConnectionString("eBike")));

            services.AddBackendPurchaseDependencies(options =>
                options.UseSqlServer(
                    Configuration.GetConnectionString("eBike")));

            services.AddSalesBackendDependencies(options =>
                options.UseSqlServer(
                    Configuration.GetConnectionString("eBike")));

            services.AddReceivingBackendDependencies(options =>
                options.UseSqlServer(
                    Configuration.GetConnectionString("eBike")));
            ...
            services.AddRazorPages(options =>
            {
                ...
                options.Conventions.AuthorizeFolder("/SalesReturns")
